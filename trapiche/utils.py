...
# AUTOGENERATED! DO NOT EDIT! File to edit: ../nbs/00.0.1_utils.ipynb.
...
def sanity_check_otus_annot_file(filepath):
    ...
    # valid_headers = {'# Constructed from biom file', '# OTU ID', 'SSU_rRNA', 'taxonomy', 'taxid'}
    ...
    with open(filepath, 'r') as f:
        ...
        for ix,line in enumerate(content):
            ...
            if spl[0][0]=='#':
                ...
                # if len(spl) != 3: # as this is inconsisten across versions, remove
                    # return False
                    None
        return content
...
def parse_otus_count(
    ...
    """
    se = [" ".join(';'.join(line.strip().split('\t')[1:]).replace('__','//').split('_')).replace('//','__').replace('Candidatus ','') for line in content]
...
def sanity_check_diamond_annot_file(filepath):
    ...
    # Open the file for reading
    with gzip.open(filepath, "r") as h:
        content= [x.decode('utf8') for x in list(h)]
        ...
        for ix,line in enumerate(content):
            ...
            # Read the rest of the lines
                ...
                # Check if there are 4 fields
                # if len(line) != 17: # differences across version. remove
                    # return False
                    None
        return content


# %% ../nbs/00.0.1_utils.ipynb 7
from numpy import dot
from numpy.linalg import norm

# %% ../nbs/00.0.1_utils.ipynb 8
...
# %% ../nbs/00.0.1_utils.ipynb 22
from difflib import SequenceMatcher

# %% ../nbs/00.0.1_utils.ipynb 23
...
# %% ../nbs/00.0.1_utils.ipynb 29
import networkx as nx

# %% ../nbs/00.0.1_utils.ipynb 30
...
# %% ../nbs/00.0.1_utils.ipynb 34
def roc_preds(true,path,probs,theshold):
    " given a lineage and the probs associated with it keep positives above threshold"
    return path[np.where(probs>=thres)[0]]
    

# %% ../nbs/00.0.1_utils.ipynb 35
def semantic_distance(ru,mi,k=2):
    " calculate diatnace"
    return((ru**k)+(mi**k))**(1/k)

# %% ../nbs/00.0.1_utils.ipynb 36
def info_theoretic_metrics(T,P,graph):
    ...
    if w_precision==float('nan'): ## solve nan isue with null predictions (root)
        w_precision==0
    ...
    return precision,recall,misinformation,remaining_uncertainty,w_precision,w_recall

# %% ../nbs/00.0.1_utils.ipynb 37
...
# %% ../nbs/00.0.1_utils.ipynb 38
def set_info_theoretic_metrics(true,pred,graph,k=1):
    " calculate info_theoretical metrics for a set of predictions "
    precision,recall,misinformation,remaining_uncertainty,w_precision,w_recall = list(zip(*[info_theoretic_metrics(T,P,graph) for T,P in zip(true,pred) ]))
    ...
    wru = sum([ tic*_ru  for tic,_ru in zip(ture_info_contents,remaining_uncertainty) ])  / pd.Series(ture_info_contents).sum()

    # wmi(true,pred): "calculate weighted misinformation for a set of terms. Downweight shallow terms"
    wmi = sum([ tic*_mi  for tic,_mi in zip(ture_info_contents,misinformation) ])  / pd.Series(ture_info_contents).sum()
