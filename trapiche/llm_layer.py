# AUTOGENERATED! DO NOT EDIT! File to edit: ../nbs/01.00.05_LLM_layer.ipynb.

# %% auto 0
__all__ = ['TAG', 'DATA_DIR', 'TMP_DIR', 'gpus', 'models_dir', 'model', 'tokenizer', 'prompt_tree', 'terrestrial_features',
           'animal_features', 'APENDIX', 'ENA_API', 'set_piority_terms', 'min_annots', 'mgnify_mixed_predictions_file',
           'llm_work', 'navigate_tree', 'get_lineages', 'lineages_from_text', 'get_text_from_enaapi', 'get_acc_desc']

# %% ../nbs/01.00.05_LLM_layer.ipynb 3
import glob
import os
import json
import re
...
# %% ../nbs/01.00.05_LLM_layer.ipynb 4
...
# %% ../nbs/01.00.05_LLM_layer.ipynb 8
from trapiche.goldOntologyAmendments import (
    biome_original_graph,
    biome_graph,
    gold_categories,
)


# %% ../nbs/01.00.05_LLM_layer.ipynb 9
TAG = "llm_layer"


# %% ../nbs/01.00.05_LLM_layer.ipynb 10
DATA_DIR = f"{config.basedir}/data/{TAG}"


# %% ../nbs/01.00.05_LLM_layer.ipynb 11
os.makedirs(DATA_DIR, exist_ok=True)


# %% ../nbs/01.00.05_LLM_layer.ipynb 12
TMP_DIR = f"{DATA_DIR}/temp"


# %% ../nbs/01.00.05_LLM_layer.ipynb 20
...
# %% ../nbs/01.00.05_LLM_layer.ipynb 21
gpus = torch.cuda.is_available()
print(f"GPUs available: {gpus}")
# cpus = False


# %% ../nbs/01.00.05_LLM_layer.ipynb 24
models_dir = f"{DATA_DIR}/flan-t5-xxl"

# %% ../nbs/01.00.05_LLM_layer.ipynb 26
model = AutoModelForSeq2SeqLM.from_pretrained(models_dir, torch_dtype=torch.bfloat16)
tokenizer = AutoTokenizer.from_pretrained(models_dir)

# %% ../nbs/01.00.05_LLM_layer.ipynb 27
model = model.to("cuda:0") if gpus == True else model


# %% ../nbs/01.00.05_LLM_layer.ipynb 28
def _clean():
...
# %% ../nbs/01.00.05_LLM_layer.ipynb 29
def llm_work(tokenizer, sentence, model, gpus=False, max_new_tokens=4, inspect_mode=False):
...
# %% ../nbs/01.00.05_LLM_layer.ipynb 33
prompt_tree = {
    "a": """
...
# %% ../nbs/01.00.05_LLM_layer.ipynb 34
...
# %% ../nbs/01.00.05_LLM_layer.ipynb 35
animal_features = {
...
# %% ../nbs/01.00.05_LLM_layer.ipynb 36
APENDIX = """
A) Yes, or
b) No?
Description: {}
...
# %% ../nbs/01.00.05_LLM_layer.ipynb 43
def navigate_tree(description, inspect_mode=False):
    """navigate the question tree, to find potential confusions"""
    worked = None
    result = {}
    if worked in [None]:
        _asw = llm_work(
            tokenizer, prompt_tree["j"].format(description), model, gpus=gpus, inspect_mode=inspect_mode,
        )
        _asw = _asw.strip(")").upper()
        if _asw == "A":
            result.setdefault("Food production", [])
        if _asw == "B":
            result.setdefault("Wastewater", [])
        if _asw == "C":
            result.setdefault("Bioreactor", [])
        if _asw == "D":
            _asw2 = llm_work(
                tokenizer, prompt_tree["i"].format(description), model, gpus=gpus, inspect_mode=inspect_mode,
            )
            ...
            if "Environmental" in result:
                _asw3 = llm_work(
                    tokenizer, prompt_tree["ia"].format(description), model, gpus=gpus, inspect_mode=inspect_mode,
                )
                ...
                if _asw3 == "C":
                    ...
                    _asw31 = llm_work(
                        tokenizer,
                        prompt_tree["f"].format(description),
                        model,
                        gpus=gpus, inspect_mode=inspect_mode,
                    )
                    if _asw31 == "A":
                        result.setdefault("Host-associated", [])
                ...
                if "Aquatic" in result:
                    _asw4 = llm_work(
                        tokenizer,
                        prompt_tree["iab"].format(description),
                        model,
                        gpus=gpus, inspect_mode=inspect_mode,
                    )
                    _asw4 = _asw4.strip(")").upper()
                    if _asw4 == "A":
                        result.setdefault("Marine", [])
                    if _asw4 == "B":
                        result.setdefault("Estuary", [])
                    if _asw4 == "C":
                        result.setdefault("Non-marine Saline and Alkaline", [])
                    if _asw4 == "D":
                        result.setdefault("Lentic", [])
                    if _asw4 == "E":
                        result.setdefault("Freshwater", [])
                    if _asw4 == "F":
                        result.setdefault("Thermal springs", [])

            if "Host-associated" in result:
                _asw2 = llm_work(
                    tokenizer, prompt_tree["ib"].format(description), model, gpus=gpus, inspect_mode=inspect_mode,
                )
                _asw2 = _asw2.strip(")").upper()
                if _asw2 == "A":
                    result.setdefault("Animal", [])
                if _asw2 == "B":
                    result.setdefault("Plants", [])
                if _asw2 == "C":
                    result.setdefault("Fungi", [])

    if "Animal" in result:
        _asw = llm_work(
            tokenizer, prompt_tree["a"].format(description), model, gpus=gpus, inspect_mode=inspect_mode,
        )
        _asw = _asw.strip(")").upper()
        if _asw == "A":
        ...
        if "Non-Defined" in result:
            _asw2 = llm_work(
                tokenizer, prompt_tree["b"].format(description), model, gpus=gpus, inspect_mode=inspect_mode,
            )
            _asw2 = _asw2.strip(")").upper()
        ...
        if "Non-Defined" in result:
            _asw3 = llm_work(
                tokenizer, prompt_tree["h"].format(description), model, gpus=gpus, inspect_mode=inspect_mode,
            )
            _asw3 = _asw3.strip(")").upper()

            if _asw3 == "A":
                result.setdefault("Birds", [])
            if _asw3 == "B":
                result.setdefault("Insecta", [])
            if _asw3 == "C":
                result.setdefault("Fish", [])
            if _asw2 == "D":
                None
            else:
                del result["Non-Defined"]
    ...
    if "Terrestrial" in result:
        for feat, question in terrestrial_features.items():
            ...
            asw = llm_work(tokenizer, prompt, model, gpus=gpus, inspect_mode=inspect_mode,)
            asw = asw.strip(")").upper()
            if asw == "A":
...
# %% ../nbs/01.00.05_LLM_layer.ipynb 44
        ...
        if len(v) == 0:
            ...
            for _k in v:
                try:
                    lin = [
                        l
                        for l in biome_herarchy_dct.values()
                        if k in l and _k in l.split(":")[-1]
                    ][0]
                except:
                    lin = [
                        l for l in biome_herarchy_dct.values() if k in l.split(":")[-1]
                    ][0]
                lineages.append(lin)
    return lineages


# %% ../nbs/01.00.05_LLM_layer.ipynb 45
def lineages_from_text(description):
    """finction to use in cli and modules to predict biome lineages base on text description"""
    bimoes_dct = navigate_tree(description)
    results = get_lineages(bimoes_dct)
    return results


# %% ../nbs/01.00.05_LLM_layer.ipynb 47
ENA_API = False
        ...
        while exit_flag == False:
            ...
            for lf in ["TITLE", "DESCRIPTION"]:
                try:
                    ...
                    None
            still = {
                x for x in project.split() if "PRJ" == x[:3]
            } - exit_bag  ### ADD PROJECTS DESCRIPTIONS EMBEDDED IN DESCIPRION
...
# %% ../nbs/01.00.05_LLM_layer.ipynb 68
set_piority_terms = {
    "Agricultural",
    "Agricultural field",
    "Air",
...
# %% ../nbs/01.00.05_LLM_layer.ipynb 85
