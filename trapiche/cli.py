...
def main(argv: Iterable[str] | None = None) -> None:
    ...
    This mirrors the previous fastcore call_parse interface while adopting the
    argparse style used by the text classifier CLI for consistency.
    """
    ...
    if not args.no_text_prediction:
        ...
        logger.info("Making text-based predictions")
    ...
    if not args.no_taxonomy_prediction:
        ...
        for directory in input_df['taxonomy_files_dir']:
            ...
            logger.info(f"Found {len(taxonomy_files)} taxonomy files in directory {directory}")
